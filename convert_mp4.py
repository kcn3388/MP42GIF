...
parser.add_argument(
    "-A", "--apng", action='store_true',
    help="是否保存为APNG，默认否，不可与拆分、不保存GIF同时使用"
)
parser.add_argument(
    "-IS", "--imagesize",
    type=str,
    default="",
    help="按指定尺寸保存，会覆盖比例参数"
)
parser.add_argument(
    "-DS", "--disposal",
    type=int,
...
def convert_images_to_gif(output_file, resize_ratio: Tuple[int, int] = None):
    ...
    if resize_ratio:
        ...
        frames = copy.deepcopy(frames_origin)
    # frames_rgba = []
...
def convert_mp4_to_gif(load, save):
    ...
    if args.imagesize:
        try:
            resize_ratio: Tuple[int, int] = tuple([int(x) for x in args.imagesize.split("*")])  # noqa
        except Exception:
            raise "尺寸输入错误，格式应为x*y"
    else:
        resize_ratio = None  # noqa
        ...
        if os.path.isdir(load_path):
            if args.combine:
                ...
                if not re.match(r"\.mp4", image_file_name[-1], re.IGNORECASE):
                    if re.match(r"(\.webp|\.gif)", image_file_name[-1], re.IGNORECASE):
                        is_img = True
                    else:
                        continue
